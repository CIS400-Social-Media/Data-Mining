...
sentiment_mapping = {'Positive': 7.0, 'Negative': 0.0, 'Neutral': 3.5}

# Map sentiment categories to numerical values
youtube_df['Sentiment'] = youtube_df['Sentiment'].map(sentiment_mapping)

# Load the WHR dataset (1 is most free and 7 is least free)
...
for category in youtube_df['Category'].unique():
    ...
    # Calculate correlation between freedom index and sentiment for the current category
    ...
    merged_df.to_csv(f"./output/{category}_freedom.csv")

# Print correlation results for each category
for category, correlation in correlation_results.items():
    print(f"Correlation between Freedom Index and Sentiment Analysis for {category}: {correlation}")
...
correlation_df.to_csv("category_freedom_correlation.csv")